...
import { UsersIcon, UserPlusIcon, ArrowTopRightOnSquareIcon, MagnifyingGlassIcon, ChevronLeftIcon, ChevronRightIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
...
export default function ReferralsPage() {
  ...
  const fetchReferralStats = async () => {
    try {
      setIsLoading(true);
      const data = await getReferralStatistics(debouncedSearchQuery, page, limit);
      setStats(data);
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center">
            ...
            <CountBadge count={isLoading ? 0 : stats.totalReferrals} />
          </div>
          <button
                  ...
                  <tr key={referrer._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        ${referrer.totalReferralAmount ? referrer.totalReferralAmount.toFixed(2) : '0.00'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <button
                        onClick={() => handleViewCustomer(referrer._id)}
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{Math.min(page * limit, stats.totalReferrals)}</span> of{' '}
                <span className="font-medium">{stats.totalReferrals}</span> results
              </p>
            </div>
            <div>
